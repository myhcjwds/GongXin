...
sheet = workbook.add_sheet('商标')
title = ['序号', '企业序号', '统一社会信用代码', '商标序号', '商标图片', '商标名称', '注册号', '状态', '分类', '申请日期']
# 序号
row_num = 0
...
for filename in os.listdir(word_folder_path):
    if filename.endswith('.docx'):
        ...
        for table in doc.tables:
            if len(table.rows[0].cells) == 7 and table.rows[0].cells[1].text == "商标图片":
...
cls_path = os.path.join(output_folder, '商标.xlsx')
